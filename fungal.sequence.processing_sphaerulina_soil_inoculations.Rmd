...
filtforward_its_soil_p2_no_missing<-subset(filtforward_its_soil_p2,basename(filtforward_its_soil_p2)%in%list.files(file.path(soil_its_path_p2, "filtered")))

filtreverse_its_soil_p2_no_missing<-subset(filtreverse_its_soil_p2,basename(filtreverse_its_soil_p2)%in%list.files(file.path(soil_its_path_p2, "filtered")))

# Generating quality plots for the forward and reverse reads after quality filtering
filtered_qualplot_fwd_its_soil_p2<-plotQualityProfile(filtforward_its_soil_p2_no_missing, aggregate=TRUE)
filtered_qualplot_fwd_its_soil_p2
filtered_qualplot_rev_its_soil_p2<-plotQualityProfile(filtreverse_its_soil_p2_no_missing, aggregate=TRUE)
...
derepforward_its_soil_p2<-derepFastq(filtforward_its_soil_p2_no_missing,verbose=TRUE)
derepreverse_its_soil_p2<-derepFastq(filtreverse_its_soil_p2_no_missing,verbose=TRUE)

# Learning errors for forward and reverse reads. 
errorforward_its_soil_p2<-learnErrors(derepforward_its_soil_p2, multithread = FALSE)
errorreverse_its_soil_p2<-learnErrors(derepreverse_its_soil_p2,multithread = FALSE)
...
Denoising reads and inferring ASVs using the dada2 function. Using *pool=TRUE* to improve estimations of alpha-diversity [Bardenhorst et al. 2022](https://www.sciencedirect.com/science/article/pii/S2001037021005456).

```{r}
# Denoising forward reads with dada2
dadaforwardreads_its_soil_p2<-dada(derepforward_its_soil_p2,err=errorforward_its_soil_p2,multithread = FALSE,pool=TRUE)

# Denoisng reverse reads with dada2
dadareversereads.ds_its_soil_p2<-dada(derepreverse_its_soil_p2,err=errorreverse_its_soil_p2,multithread = FALSE,pool=TRUE)
```
...
# Removing files that were not written as a result of no reads passing filter
filtforward_its_soil_p1_no_missing<-subset(filtforward_its_soil_p1,basename(filtforward_its_soil_p1)%in%list.files(file.path(soil_its_path_p1, "filtered")))

filtreverse_its_soil_p1_no_missing<-subset(filtreverse_its_soil_p1,basename(filtreverse_its_soil_p1)%in%list.files(file.path(soil_its_path_p1, "filtered")))
...
# Learning errors for forward and reverse reads. 
errorforward_its_soil_p1<-learnErrors(derepforward_its_soil_p1, multithread = FALSE)
errorreverse_its_soil_p1<-learnErrors(derepreverse_its_soil_p1,multithread = FALSE)
...
```{r}
# Denoising forward reads with dada2
dadaforwardreads_its_soil_p1<-dada(derepforward_its_soil_p1,err=errorforward_its_soil_p1,multithread = FALSE,pool=TRUE)

# Denoisng reverse reads with dada2
dadareversereads.ds_its_soil_p1<-dada(derepreverse_its_soil_p1,err=errorreverse_its_soil_p1,multithread = FALSE,pool=TRUE)
```
...
# Removing files that were not written as a result of no reads passing filter
filtforward_its_soil_initial_no_missing<-subset(filtforward_its_soil_initial,basename(filtforward_its_soil_initial)%in%list.files(file.path(soil_its_path_initial, "filtered")))

filtreverse_its_soil_initial_no_missing<-subset(filtreverse_its_soil_initial,basename(filtreverse_its_soil_initial)%in%list.files(file.path(soil_its_path_initial, "filtered")))
...
# Learning errors for forward and reverse reads. 
errorforward_its_soil_initial<-learnErrors(derepforward_its_soil_initial, multithread = FALSE)
errorreverse_its_soil_initial<-learnErrors(derepreverse_its_soil_initial,multithread = FALSE)
...
```{r}
# Denoising forward reads with dada2
dadaforwardreads_its_soil_initial<-dada(derepforward_its_soil_initial,err=errorforward_its_soil_initial,multithread = FALSE,pool=TRUE)

# Denoisng reverse reads with dada2
dadareversereads.ds_its_soil_initial<-dada(derepreverse_its_soil_initial,err=errorreverse_its_soil_initial,multithread = FALSE,pool=TRUE)
```
...
seqtab.nochim_its_soil<-removeBimeraDenovo(merged_sequence_table,method="consensus", multithread=FALSE, verbose=TRUE)
...
Assigning taxonomy to ASVs using the [UNITE Eukaryote database](https://unite.ut.ee/repository.php). Then removing ASVs unassigned to a fungal phyla or identified to a kingdom other than fungi.  
...
Filtering the ASV abundance table by merging the ASV table with the taxonomy assignments. This will remove ASVs from the count table that were filtered out based on taxonomy.
...
save.image(file="~/OneDrive - Oak Ridge National Laboratory/septoria_in_a_bag/its_data/analyses_original_data_february_2024/feb_22_2024_fungal_its2_sequence_processing.RData")
```
